...
final class MetricsTests: XCTestCase {
    ...
    func testDispatchJobCounter() async throws {
        ...
        let counter = try XCTUnwrap(Self.testMetrics.recorders.withLockedValue { $0 }["swift_jobs"] as? TestRecorder)
        ...
        XCTAssertEqual(counter.dimensions[1].0, "job_status")
    ...
    func testFailedJobs() async throws {
        ...
        XCTAssertEqual(counter.values.withLockedValue { $0 }[0].1, 1)
        XCTAssertEqual(counter.dimensions[0].0, "job_id")
        XCTAssertNotNil(counter.dimensions[0].1)
        XCTAssertEqual(counter.dimensions[1].0, "job_name")
        XCTAssertEqual(counter.dimensions[1].1, "testFailedJobs()")
        XCTAssertEqual(counter.dimensions[2].0, "job_status")
        XCTAssertEqual(counter.dimensions[2].1, "failed")
        // TODO: test retry gauge
//        let retryCounter = try XCTUnwrap(Self.testMetrics.recorders.withLockedValue { $0 }["swift_jobs"] as? TestRecorder)
//        XCTAssertEqual(retryCounter.values.withLockedValue { $0 }.count, 1)
//        XCTAssertEqual(retryCounter.values.withLockedValue { $0 }[0].1, 1)
//        XCTAssertEqual(retryCounter.dimensions[1].0, "job_name")
//        XCTAssertEqual(retryCounter.dimensions[1].1, "testFailedJobs()")
//        XCTAssertEqual(retryCounter.dimensions[2].0, "job_status")
//        XCTAssertEqual(retryCounter.dimensions[2].1, "retrying")
        let timer = try XCTUnwrap(Self.testMetrics.timers.withLockedValue { $0 }["swift_jobs_duration"] as? TestTimer)
        XCTAssertEqual(timer.dimensions[1].0, "job_status")
    ...
    func testJobExecutionTime() async throws {
        ...
        XCTAssertEqual(timer.dimensions[0].0, "job_name")
